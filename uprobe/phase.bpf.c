...
SEC("kprobe/__x64_sys_execve")
int BPF_KPROBE(Exec_docker)
{
    char comm[TASK_COMM_LEN];
    bpf_get_current_comm(&comm, sizeof(comm)); 
    if(bpf_strncmp(comm,6,"docker")==0){
        struct task_struct *task;
        task = (struct task_struct *)bpf_get_current_task();
        int ppid = BPF_CORE_READ(task, real_parent, tgid);
        int pid = bpf_get_current_pid_tgid()>>32;
...
//<github.com/opencontainers/runc/libcontainer.(*LinuxFactory).Create> 1.1.8
...
int BPF_UPROBE(container_create)
{
    ...
    bpf_printk("container_Create start! Pid is %d,PPid is %d,cmd is %s!",pid,ppid,comm);
...
int BPF_UPROBE(container_config)
{
    ...
    task = (struct task_struct *)bpf_get_current_task();
    ppid = BPF_CORE_READ(task, real_parent, tgid);
    bpf_printk("CreateLibcontainerConfig! Pid is %d,PPid is %d,cmd is %s!",pid,ppid,comm); 
    //print_func(ctx);
    void *arg;
    char *rootfs;
...
int BPF_UPROBE(bootstrap1)
{
    ...
    task = (struct task_struct *)bpf_get_current_task();
    ppid = BPF_CORE_READ(task, real_parent, tgid);
    bpf_printk("runc's bootstrap start! Pid is %d,PPid is %d,cmd is %s!",pid,ppid,comm);
...
int BPF_UPROBE(runc_init)
{
    ...
    task = (struct task_struct *)bpf_get_current_task();
    ppid = BPF_CORE_READ(task, real_parent, tgid);
    bpf_printk("runc's standard init start! Pid is %d,PPid is %d,cmd is %s!",pid,ppid,comm);
    
    return 0;
}
//<github.com/opencontainers/runc/libcontainer.newContainerInit> 1.1.8
//containerInit函数搜集容器的全部config，用于容器进程的创建,runc/libcontainer/init_linux.go下:
SEC("uprobe")
...
int BPF_UPROBE(join_namespaces)
{
    ...
    task = (struct task_struct *)bpf_get_current_task();
    ppid = BPF_CORE_READ(task, real_parent, tgid);
    bpf_printk("join_namespaces's setns start! Pid is %d,PPid is %d,cmd is %s!",pid,ppid,comm);
...
int handle_setns(struct trace_event_raw_sys_enter *ctx)
{
    ...
    bpf_printk("Setns start!pid is %d,ppid is %d,comm is %s!",pid,ppid,comm);
    return 0;
}
